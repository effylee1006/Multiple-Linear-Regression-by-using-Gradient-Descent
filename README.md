...
### 2.4 梯度下降的實現
因馬達出的力 = 庫倫摩擦力根據方向 + 黏滯力 + 慣性力 + 重力，即𝝉_𝒎=𝒔𝒈𝒏(𝝎)×𝒇_𝒄+𝝎×𝑩+𝑱×𝜶+𝝉_𝒈，而目前的訊號數據僅包含，速度及電流，故須透過預處理，𝒔𝒈𝒏(𝝎)及𝜶透過原始資料轉換出來，以下介紹預處理方法及步驟 :
(1)移除速度𝝎 最大值*0.01與最小值*0.01 (正負20rpm)之間的值
(2)速度𝝎做一次插分得到 加速度 𝜶
(3)對acc做平均濾波 : 去除 𝜶 跳動的資料點，每32點做平均濾波，可移除預測中的隨機波動
(4)透過加速度的最大值與最小值的閥值，移除acc的outlier
(5)根據速度決定𝒔𝒈𝒏(𝝎)，若是spd>0，sgn為1
(6)去除acc零速附近的值 ，把acc等於0的值拿掉

預利用利用𝝉_𝒎, 𝒔𝒈𝒏(𝝎), 𝝎, 𝜶，來估計𝒇_𝒄, 𝑩, 𝑱,𝝉_𝒈四個參數，進而得到預測值電流，目前使用線性迴歸來預測參數，而本專案欲改用梯度下降法，進行參數估計，以下介紹我們如何使用梯度下降法估計參數。
